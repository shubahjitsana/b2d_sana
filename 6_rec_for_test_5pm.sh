#!/bin/bash
# takes command as: ./6_rec_for_test.sh 230 438 111 106 462
...
let rec_start_charged=${1:-230}-1        #as for loop starts from next numbering
let rec_start_uubar=${2:-438}-1
let rec_start_ddbar=${3:-111}-1
let rec_start_ssbar=${4:-106}-1
let rec_start_ccbar=${5:-462}-1
let rec_start=0
let rec_start=0
...
do
    ...
    # path of input files
    input_datapath='/group/belle2/dataprod/MC/MC14ri_a'
